...
class SupportRequest(models.Model):
    ...
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    support_file =  models.ManyToManyField('GenericFile', blank=True, related_name="file_session")

class SupportOffer(models.Model):
    supporter = models.ForeignKey(User, null=True, blank=True, on_delete=models.CASCADE, related_name='supporter_session')
    request = models.ForeignKey(SupportRequest, on_delete=models.CASCADE, related_name="request_session", null=True, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
...
class GenericNote(models.Model):
    author = models.ForeignKey(User, null=True, on_delete=models.CASCADE, related_name='item_note')
    note = models.TextField(null=True, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)

def generic_file_path(instance, filename):    
    path= "generic/"+str(instance.pk)+"/"+filename
    return path

class GenericFile(models.Model):
    file_name = models.CharField(max_length=256, null=True, blank=True)
    generic_file=models.FileField(max_length=500, null=True, blank = True, upload_to = generic_file_path)
    created_at = models.DateTimeField(auto_now_add=True)
    def filename(self):
        return os.path.basename(self.generic_file.name)

# class SafetySheetTemplate(models.Model):
#     company = models.ForeignKey(Company, null=True, blank=True, on_delete=models.CASCADE, related_name="user_company")
#     name = models.TextField(null=True, blank=True)
#     created_at = models.DateTimeField(auto_now_add=True)    

# class SafetyBoolean(models.Model):
#     sheet = models.ForeignKey(SafetySheetTemplate, null=True, blank=True, on_delete=models.CASCADE, related_name="safety_template_boolean")
#     bool_title = models.TextField(null=True, blank=True)
    
# class SafetyText(models.Model):
#     sheet = models.ForeignKey(SafetySheetTemplate, null=True, blank=True, on_delete=models.CASCADE, related_name="safety_template_text")
#     text_title = models.TextField(null=True, blank=True)


class ScopeItem(models.Model):
    job = models.ForeignKey(Job, null=True, blank=True, on_delete=models.CASCADE, related_name="job_scope_item")
    description = models.TextField(null=True, blank=True)
    is_complete=models.BooleanField(default=False)
    created_by = models.ForeignKey(User, null=True, on_delete=models.CASCADE, related_name='user_scope_item')
    created_at = models.DateTimeField(auto_now_add=True)
    

class ScopeTask(models.Model):
    task_item = models.ForeignKey(ScopeItem, null=True, blank=True, on_delete=models.CASCADE, related_name="scope_item_task")
    task_title = models.TextField(null=True, blank=True)
    is_complete= models.BooleanField(default=False)
    created_by = models.ForeignKey(User, null=True, on_delete=models.CASCADE, related_name='user_scope_task')
    created_at = models.DateTimeField(auto_now_add=True)
        
class SafetySheet(models.Model):
    job = models.ForeignKey(Job, null=True, blank=True, on_delete=models.CASCADE, related_name="job_safety_sheet")
    created_by = models.ForeignKey(User, null=True, on_delete=models.CASCADE, related_name='user_safety_sheet')
    created_at = models.DateTimeField(auto_now_add=True)
    date = models.DateField(default=date.today)

class SafetyHazard(models.Model):
    safety_sheet = models.ForeignKey(SafetySheet, null=True, blank=True, on_delete=models.CASCADE, related_name="sheet_hazard")
    ...
    is_complete= models.BooleanField(default=False)
    created_by = models.ForeignKey(User, null=True, on_delete=models.CASCADE, related_name='user_hazard')
    scope_item = models.ManyToManyField(ScopeItem, blank=True, related_name="scope_hazard")
    created_at = models.DateTimeField(auto_now_add=True)

class LotoDescription(models.Model):
    safety_sheet = models.ForeignKey(SafetySheet, null=True, blank=True, on_delete=models.CASCADE, related_name="sheet_loto")
    location = models.TextField(null=True, blank=True)
    description = models.TextField(null=True, blank=True)
    ground_location = models.TextField(null=True, blank=True)
    is_grounded = models.BooleanField(default=False)

class SafetyBoolean(models.Model):
    safety_sheet = models.ForeignKey(SafetySheet, null=True, blank=True, on_delete=models.CASCADE, related_name="sheet_bool")
    label = models.TextField(null=True, blank=True)

class Signature(models.Model):
    user = models.ForeignKey(User, null=True, on_delete=models.CASCADE, related_name='sheet_signature')
    manual_entry = models.TextField(null=True, blank=True)
