...
def add_job_note(request, job_id):
    ...
    return render(request, "jobs/error.html", {"message": "Error! Note not added. Must Contain letters, numbers, or symbols. If problem persists, contact admin"})

def quick_job_note(request):
    if not request.user.is_authenticated:
        return render(request, "jobs/login.html", {"message": None})
    if "add_note_pk" in request.POST:
        if request.POST["add_note_pk"] and request.POST["add_note_pk"].strip():
            job = Job.objects.get(pk = request.POST["add_note_pk"])
            if "new_note" in request.POST:
                if request.POST["new_note"] and request.POST["new_note"].strip():
                    job_note_posted = request.POST["new_note"]
                    job_note = JobNotes.objects.create(note=job_note_posted, job=job, author=request.user)
    ...
    return render(request, "jobs/error.html", {"message": "Server Error. Note not added. Contact Admin"})
    ...
    if "add_note" in request.POST:
        if request.POST["add_note"] and request.POST["add_note"].strip():
            ...
            job_note = JobNotes.objects.create(note=job_note_posted, job=job, author=request.user)
            return HttpResponseRedirect(reverse("job", args=(job_id, ))+"#job_notes")
    return render(request, "jobs/error.html", {"message": "Error. Note not added. Possible content error. Must Contain letters, numbers, or symbols. Contact admin if issue persists"})
...
def quick_job_file(request):
    if not request.user.is_authenticated:
        return render(request, "jobs/login.html", {"message": None})    
    if "add_file_pk" in request.POST:
        if request.POST["add_file_pk"] and request.POST["add_file_pk"].strip():
            try:
                job = Job.objects.get(pk = request.POST["add_file_pk"])
            except KeyError:
                return render(request, "jobs/error.html", {"message": "Server error: No Selection. Contact admin"})
            except Job.DoesNotExist:
                return render(request, "jobs/error.html", {"message": "Server error: Invalid Job Selection. Contact admin"})
            if 'file_add' in request.FILES:
                file_add = request.FILES["file_add"]
                job_folder=JobFolder(job=job)
                job_folder.job_file = file_add
...
def quick_fsr_add(request):
    if not request.user.is_authenticated:
        return render(request, "jobs/login.html", {"message": None})
    if "add_fsr_job_pk" in request.POST:
        if request.POST["add_fsr_job_pk"] and request.POST["add_fsr_job_pk"].strip():
            job = Job.objects.get(pk = request.POST["add_fsr_job_pk"])
            if "add_fsr_pk" in request.POST:
                if request.POST["add_fsr_pk"] and request.POST["add_fsr_pk"].strip():
                    fsr=UserProperties.objects.get(pk=add_fsr_pk)
                    job.user_properties.add(fsr)
                    job.save()
                    return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
            return render(request, "jobs/error.html", {"message": "Server Error. Crew member not added. Contact admin"})
    return render(request, "jobs/error.html", {"message": "Server Error. Crew member not added. Contact Admin"})
...
def create_request(request):
    ...
    eq = request.POST["eq_id"]
    specialists = request.POST.getlist('specialists')
    details = request.POST["details"].strip()
    support_request = SupportRequest(details=details, description=description, requester = requester, equipment=Equipment.objects.get(id = eq))
    support_request.save()
    for specialist in specialists:
        specialist = UserProperties.objects.get(pk=specialist)        
        offer = SupportOffer(supporter = specialist.user, request = support_request)
        offer.save()
        session = SupportSession(offer = offer)
        session.save()
    return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
...
def issue_resolved_toggle(request):
    ...
    return JsonResponse({"response": request.is_resolved})

def session_request(request):
    specialist_id = request.POST.get('specialist_id', None)
    valid = True
    equipment_id = request.POST.get('equipment_id', None)
    request_id = request.POST.get('request_id', None)
    if equipment_id and request_id and specialist_id:
        equipment = Equipment.objects.get(pk=equipment_id)
        support_request = SupportRequest.objects.get(pk=request_id)
        specialist = UserProperties.objects.get(pk=int(specialist_id))
        if SupportOffer.objects.filter(supporter = specialist.user, request = support_request).exists():
            return JsonResponse({"fail": 'already exists'})
    ...
    return JsonResponse({"some response": 'specialist_id'})
