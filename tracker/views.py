...
def job(request, job_id, new_obj=None, obj_type=None):
    if not request.user.is_authenticated:
        return render(request, "jobs/login.html", {"message": None})
    try:
        user_properties=UserProperties.objects.get(user=request.user)
...
def create_type_view(request, job_id=''):
    print("000000000")
    if not request.user.is_authenticated:
        return render(request, "jobs/login.html", {"message": None})
...
def create_model_view(request, job_id=''):
...
def equipment(request, equipment_id):
    ...
    try:
        ...
        model_support=UserProperties.objects.filter(equipment_models_supported = equipment.equipment_model, company = user_properties.company)
        type_support = UserProperties.objects.filter(equipment_types_supported = equipment.equipment_type, company = user_properties.company)
    ...
    if request.user.is_authenticated:
        for ea in support_users:
            print(ea.user.supporter_session.all())
...
def agwell(request, well_id):
    ...
    if request.user.is_authenticated and properties.company and well:
        ...
        return render(request, "jobs/error.html", {"message": "Your profile must be associated with the Agriculture portion of the App to view this information. If you don't have the company key for this, contact us.  Click Edit Profile above to add a key."})
    # except:
    #     # return render(request, "jobs/jobs.html", context)
...
# def safety_sheet_editor(request):
#     if not request.user.is_authenticated:
#         return render(request, "jobs/login.html", {"message": None})        
#     user_properties = UserProperties.objects.get(user=request.user)
#     if not user_properties.is_company_admin:
#         return render(request, "jobs/error.html", {"message": "You are not an admin for your company! Contact Albert Withers to request admin access. Cell: 210-303-0471 Email: albert@blueskysw.com"})
    

#     return render(request, "jobs/safety_sheet_editor.html", context)
